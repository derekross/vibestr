...
import { VIBE_CODERS_COMMUNITY_ID, extractCommunityIdFromPost } from '@/lib/community';
...
export function useNotifications() {
  ...
  const query = useQuery({
    ...
    queryFn: async (c) => {
      ...
      try {
        // Get reactions and replies to user's content from the vibe coding community
        const [reactions, replies] = await Promise.all([
          // Get reactions to user's posts
          nostr.query([{
            kinds: [7], // Reaction events
            '#p': [user.pubkey], // Reactions mentioning the user
            '#t': ['vibe-coding'], // Filter for vibe-coding community posts
            limit: 100,
          }], { signal }),
        ...
        filteredReactions.forEach(reaction => {
          ...
          // Only include reactions to user's own content from the vibe coding community
          if (targetEvent && targetEvent.pubkey === user.pubkey) {
            const communityId = extractCommunityIdFromPost(targetEvent);
            if (communityId === VIBE_CODERS_COMMUNITY_ID) {
              notifications.push({
                id: reaction.id,
                type: 'reaction',
                event: reaction,
        ...
        filteredReplies.forEach(reply => {
          ...
          // Only include replies to user's own content from the vibe coding community
          if (targetEvent && targetEvent.pubkey === user.pubkey) {
            const communityId = extractCommunityIdFromPost(targetEvent);
            if (communityId === VIBE_CODERS_COMMUNITY_ID) {
              notifications.push({
