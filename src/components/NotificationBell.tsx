...
function NotificationItem({ notification, onRead, onNavigate }: {
  ...
  const getNotificationText = () => {
    if (notification.type === 'reaction') {
      ...
      if (content === '+' || content === '') {
        return 'liked your vibe coding post';
      } else if (content === '-') {
        return 'disliked your vibe coding post';
      } else {
        return `reacted ${content} to your vibe coding post`;
      }
    } else {
      return 'replied to your vibe coding post';
    }
  };
      ...
      <div className="flex items-start space-x-3 w-full">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-muted-foreground">• Vibe Coders</span>
            {!notification.read && (
...
export function NotificationBell() {
  ...
  const handleNotificationNavigate = (notification: Notification) => {
    ...
    if (notification.type === 'reply') {
      // Navigate to the reply itself - PostPage will provide thread navigation
      navigate(`/post/${notification.event.id}`);
    } else if (notification.type === 'reaction' && notification.targetEvent) {
      // Navigate to the post that was reacted to
      navigate(`/post/${notification.targetEvent.id}`);
    } else {
      // Fallback: navigate to the notification event itself
      navigate(`/post/${notification.event.id}`);
    }
  };
